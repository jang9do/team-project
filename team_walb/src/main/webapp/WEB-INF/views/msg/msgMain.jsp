	...
	<div id="wrap">
		...
		<div id="fakeDiv"></div>
			...
			<div class="fakeDiv"></div>
				...
				<div class="msglist">
						<c:forEach var="mes" items="${msgReceiveList }">
							<!-- 쪽지 1개 둘러싸는 div -->
							<div class="mes_area">
			...
			<div id="paging">
				<c:if test="${paging.startPage>paging.blockSize }">
					<a href="${conPath }/msgMain.do?pageNum=${paging.startPage-1 }">&lt;</a>
				</c:if>
				<c:forEach var="i" begin="${paging.startPage }"
					...
					<c:if test="${i!=paging.currentPage }">
						<a href="${conPath }/msgMain.do?pageNum=${i}">${i}</a>
					</c:if>
				</c:forEach>
				<c:if test="${paging.endPage<paging.pageCnt }">
					<a href="${conPath }/msgMain.do?pageNum=${paging.endPage+1}">></a>
				</c:if>
			</div>
			</div>
		</div>
